...
import com.nilesh.lib.util.SortType;
...
public class AptHousingPage extends BasePage {
    ...
    public String getDefaultSortOption() {
        List<WebElement> selectedMenu = getAllSortMenuObjects().stream()
                .filter(element -> element.getAttribute("aria-selected").equals("true"))
                .collect(Collectors.toList());
        Assert.assertEquals(1, selectedMenu.size());
        return selectedMenu.get(0).getText().trim();
    }

    public List<String> getMenuOptions() {
        List<String> menuOptions = new ArrayList<>();

        for (WebElement ele : getAllSortMenuObjects()) {
            WebElement link = ele.findElement(By.xpath("./a"));
            menuOptions.add(link.getAttribute("text").trim());
        }
        return menuOptions;
    }
    ...
    // Private methods
    //------------------------------------------------------------------------------------------------------------------
    private List<WebElement> getAllSortMenuObjects() {
